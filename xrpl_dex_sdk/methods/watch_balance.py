...
async def watch_balance(self, params: WatchBalanceParams) -> None:
    ...
    async def balance_handler(message: Any):
        if message["type"] == "ledgerClosed":
            if (
                balance != None
                and message["reserve_base"]
                != balance["info"]["validated_ledger"]["reserve_base"]
                or message["reserve_inc"]
                != balance["info"]["validated_ledger"]["reserve_inc"]
            ):
            ...
            if transaction["TransactionType"] == "Payment":
                if (
                    ...
                    or transaction["Destination"] == account
                ):
                    return await self.fetch_balance()
                ...
                if (
                    "code" in params
                    and get_amount_currency_code(transaction["TakerGets"])
                    != params.code
                    and get_amount_currency_code(transaction["TakerPays"])
                    != params.code
                ):
                    return
                    ...
                    for affected_node in transaction["meta"]["AffectedNodes"]:
                        ...
                        if node["LedgerEntryType"] == "AccountRoot" and (
                            ...
                            if "code" in params and params.code != "XRP":
                                ...
                                "FinalFields" in node
                                and node["FinalFields"]["Account"] == account
                            ) or (
                                "NewFields" in node
                                and node["NewFields"]["Account"] == account
                            ):
                                should_refresh = True
                                break
                            ...
                            if "code" in params:
                                if "FinalFields" in node:
                                    if (
                                        node["FinalFields"]["HighLimit"]["currency"]
                                        == params.code
                                        ...
                                        break
                                    if (
                                        node["FinalFields"]["HighLimit"]["issuer"]
                                        == account
                                        and node["FinalFields"]["LowLimit"]["issuer"]
                                        == account
                                    ):
                                        should_refresh = True
                                        break
                                    ...
                                    if (
                                        node["NewFields"]["HighLimit"]["currency"]
                                        == params.code
                                        and node["NewFields"]["LowLimit"]["currency"]
                                        == params.code
                                    ):
                                        should_refresh = True
                                        break
                                    if (
                                        node["NewFields"]["HighLimit"]["issuer"]
                                        == account
                                        and node["NewFields"]["LowLimit"]["issuer"]
                                        == account
                                    ):
                                        should_refresh = True
                                        break
